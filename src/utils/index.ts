...
const grabAllAdjacentCells = (
  ...
  const { MAX_ROWS, MAX_COLS } = config;
...
export const generateCells = (config: any): Cell[][] => {
  ...
  for (let rowIndex = 0; rowIndex < MAX_ROWS; rowIndex++) {
    for (let colIndex = 0; colIndex < MAX_COLS; colIndex++) {
      ...
      const {
        ...
        bottomCell,
        bottomRightCell,
      } = grabAllAdjacentCells(config, cells, rowIndex, colIndex);
...
export const openMultipleCells = (
  ...
  const {
    ...
    bottomCell,
    bottomRightCell,
  } = grabAllAdjacentCells(config, cells, rowParam, colParam);
  ...
  if (topCell?.state === CellState.open && topCell.value !== CellValue.bomb) {
    if (topCell.value === CellValue.none) {
      newCells = openMultipleCells(config, newCells, rowParam - 1, colParam);
    } else {
  ...
  if (leftCell?.state === CellState.open && leftCell.value !== CellValue.bomb) {
    if (leftCell.value === CellValue.none) {
      newCells = openMultipleCells(config, newCells, rowParam, colParam - 1);
    } else {
